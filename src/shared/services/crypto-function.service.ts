import { CryptoFunctionService as CryptoFunctionServiceAbstraction } from "@/shared/abstractions";
import { CsprngArray } from "@/shared/types/csprng";
import { concatenateUint8Arrays, fromUtf8ToArray } from "@/shared/utils";
...
export class CryptoFunctionService implements CryptoFunctionServiceAbstraction {
  ...
  constructor() {
    ...
    this.subtle = this.crypto.subtle as SubtleCrypto;
  }

  private toBuffer(value: string | Uint8Array): Uint8Array {
    if (typeof value === "string") {
      return fromUtf8ToArray(value);
    }
  ...
  async aesGenerateKey(bitLength = 256 | 512): Promise<CsprngArray> {
    if (bitLength === 512) {
      ...
      return concatenateUint8Arrays([key1, key2]) as CsprngArray;
    }
